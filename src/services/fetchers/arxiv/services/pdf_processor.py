...
class PDFProcessor:
    ...
    def _extract_equations(self, text: str) -> List[str]:
        ...
        # LaTeX environments
        env_eq = re.findall(r"\\begin\{equation\}(.*?)\\end\{equation\}", text, re.DOTALL)
        equations.extend([e.strip() for e in env_eq if e.strip()])

        env_align = re.findall(r"\\begin\{align\}(.*?)\\end\{align\}", text, re.DOTALL)
        equations.extend([e.strip() for e in env_align if e.strip()])

        env_multline = re.findall(r"\\begin\{multline\}(.*?)\\end\{multline\}", text, re.DOTALL)
    ...
    async def extract_batch(
        ...
        for paper in papers:
            try:
                ...
                logger.error(f"Failed to extract {paper.get('paper_id', 'unknown')}: {e}")
                continue
    ...
    def __repr__(self) -> str:
        return f"PDFProcessor(processed={self._processed_count}, errors={self._error_count})"
